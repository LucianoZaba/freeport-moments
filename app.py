...
def _limpiar_temporales_huerfanos():
    ...
    if not carpeta_temp.exists():
        return
    for item in carpeta_temp.iterdir():
        try:
            if item.is_dir():
                # carpeta de partes de una subida fragmentada abandonada
                mtimes = [p.stat().st_mtime for p in item.glob("*.part")]
                ultimo = max(mtimes) if mtimes else item.stat().st_mtime
                if ultimo < limite_ts:
                    shutil.rmtree(item, ignore_errors=True)
                    logger.info(f"Limpieza: eliminada carpeta de partes huerfana {item.name}")
            elif item.is_file() and item.stat().st_mtime < limite_ts:
                item.unlink()
                logger.info(f"Limpieza: eliminado temporal huerfano {item.name}")
        except OSError as e:
            logger.warning(f"No se pudo limpiar temporal {item.name}: {e}")
...
async def api_subida_fragmentada(
    ...
    # en qué orden termina cada uno. El orden real lo da indice_bloque en el
    # nombre del archivo, no el orden de llegada.
    async with _semaforo_uploads:
        try:
            contenido = await bloque.read()
 
            def _escribir_parte():
                carpeta_partes.mkdir(parents=True, exist_ok=True)
                parte_path = carpeta_partes / f"{indice_bloque:06d}.part"
                with open(parte_path, "wb") as f:
                    f.write(contenido)
                return sum(p.stat().st_size for p in carpeta_partes.glob("*.part"))
 
            tamano_acumulado = await asyncio.to_thread(_escribir_parte)
            if tamano_acumulado > max_bytes:
    ...
    # El lock solo protege el chequeo "¿ya están todos los bloques?" + el
    # ensamblado final, para que dos bloques que terminan casi juntos no
    # ensamblen el archivo dos veces.
    lock = _locks_subida_fragmentada[id_subida]
    async with lock:
        partes = await asyncio.to_thread(lambda: sorted(carpeta_partes.glob("*.part")))
        ...
        def _ensamblar():
            with open(ruta_destino, "wb") as final:
                for parte in sorted(carpeta_partes.glob("*.part")):
                    with open(parte, "rb") as p:
                        shutil.copyfileobj(p, final, length=4 * 1024 * 1024)
            shutil.rmtree(carpeta_partes, ignore_errors=True)
            return ruta_destino.stat().st_size
 
        try:
            tamano_final = await asyncio.to_thread(_ensamblar)
        except OSError as e:
            logger.error(f"Error ensamblando archivo fragmentado {id_subida}: {e}")
